...
FILE_ADRESS = input('What is your csv file adress?\n')
MAIL_SENDER = input('What is your Google Mail Adress?\n')
GOOGLE_CLIENT_ID = input('What is your Google Client ID?\n')
GOOGLE_CLIENT_SECRET = input('What is your Google Client Secret?\n')
...
if __name__ == '__main__':
    if GOOGLE_REFRESH_TOKEN is None:
        ...
        print('Your GOOGLE_REFRESH_TOKEN:', refresh_token)
        GOOGLE_REFRESH_TOKEN = refresh_token

with open(FILE_ADRESS, 'r') as csvFile:
    reader = csv.reader(csvFile)
    for row in reader:
            send_mail(MAIL_SENDER, row[1],
              MAIL_TITLE.format(row[0],row[1]),
              MAIL_MESSAGE.format(row[0],row[1]))
csvFile.close()
